...
public class Test1013 {
    ...
    // 插入排序 最好时间复杂度O(n) 最坏时间复杂度O(n^2) 稳定
    ...
    // 希尔排序 最好时间复杂度为O(n) 最坏时间复杂度为O(n^1.3) 不稳定
    ...
    // 冒泡排序 最好时间复杂度为O(n) 最坏时间复杂度为O(n^2) 稳定
    ...
    // 堆排序 时间复杂度O(nlogn)，不怎么受数据的影响 不稳定
    ...
    // 快速排序 最好时间复杂度为O(nlogn) 最坏时间复杂度为O(n^2)
    ...
    // 归并排序 时间复杂度O(nlog2n) 不受数据影响，稳定
    // 思想：将数组分成大小相同的两半，一直分一直分，然后再进行排序，排完序再并在一起
